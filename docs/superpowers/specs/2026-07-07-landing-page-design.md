...
| Valor declarado do produto principal | **Não existe âncora inflada.** O produto principal não tem "valor riscado" — seu valor declarado é o próprio `PRODUCT_PRICE` (US$6,90). Só os 4 bônus entram no stack com valor declarado (US$6,90 cada) cobrados a US$0,00. Decisão do usuário: manter honesto, sem inflar preço do produto principal antes de haver prova social real. |
...
| # | Seção | Conteúdo |
|---|---|---|
| 1 | Hero | Headline: **"El método que convierte la culpa por el celular de tu hijo en el proyecto del que él más se enorgullece — sin que tú sepas nada de tecnología."** *(pronome "tú" — espanhol neutro, já sincronizado com o doc de arquitetura)* Subheadline: *"el Método para transformar el tiempo de pantalla de tu hijo en su primer invento"* CTA: botão **"Quiero el Método ahora →"** → `HOTMART_CHECKOUT_URL` |
| 2 | Agitação da dor + Prova social | Nomeia em espanhol as duas dores centrais: culpa de tela ("¿Cuántas veces le has dicho 'ya no más pantalla'...?") e medo de não ser da área ("Yo tampoco soy del área"). Bloco "A mí también me pasó": narrador **neutro/genérico** (decisão do usuário) — evitar autodescrição com gênero marcado (não usar "mamá convencida"/"papá convencido"); construir o arco em torno da ação e da experiência, não da identidade de quem fala (ex: *"En casa también pasábamos por esto..."*, *"Yo tampoco sabía nada de esto..."*). Arco obrigatório: problema → tentativa → descoberta → resultado. **Este bloco é o substituto de depoimentos** até haver avaliações reais. |
...
| 4 | Stack de valor | Linha por linha, sem âncora inflada: Producto principal — **`PRODUCT_PRICE`** (US$6,90, pago). Bônus 1–4 — US$6,90 cada (valor declarado), entregues por **US$0,00**. Total em bônus grátis = US$27,60. Frase-âncora honesta: *"Pagás solo el producto principal (US$6,90) y te llevás US$27,60 en bonos, gratis."* Não exibir "valor total riscado" do pacote inteiro — só os bônus usam o mecanismo de valor riscado→grátis. |
...
| 7 | FAQ | 3 objeções (lista completa — não adicionar outras sem decisão): (1) *"¿Y si no sé nada de tecnología?"* (2) *"¿Le va a gustar a mi hijo/a?"* (3) *"¿Funciona en mi país?"* — resposta (3), fechada: *"Sí. Funciona en Argentina, Chile, Perú, Colombia, Ecuador y México — pagás en la moneda de tu país, con conversión automática en el checkout."* Copy final, não depende de verificação prévia de métodos por país (decisão do usuário — Hotmart já faz a conversão automática de moeda no checkout, como confirmado na pesquisa de concorrência). |
...
- Escrever o arco completo do bloco "A mí también me pasó" em voz neutra/genérica (decisão fechada) antes de implementar a Seção 2
